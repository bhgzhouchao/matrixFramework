...
import org.matrix.framework.core.settings.ElasticSearchSettings;


public abstract class ElasticSearchAccess extends JestHelper {
    ...
    public <T> PageModel<T> query(String eql, String indexName, String indexType, Class<T> clazz, Integer pageNo, Integer pageSize) throws Exception {
        Search.Builder search = new Search.Builder(eql);
        search = search.addIndex(indexName).addType(indexType);
        search = search.setParameter("from", (pageNo - 1) * pageSize).setParameter("size", pageSize);
        return getJestResultConvert().convert(elasticSearchClient.execute(search.build()), pageNo, pageSize, clazz);
    }
